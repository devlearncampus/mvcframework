 ...
 * 	
 * MVC주요 내용? 
 * 				디자인영역과 로직(모델) 영역은 완전히 분리시켜야 , 유지보수성에 좋아진다
 * 
 * Model2란? 
 * 				JavaEE 분야에서 구현한 MVC 패턴을 가리킴 
 * 				즉 javaEE 분야(웹)에서 애플리케이션을 개발할때 디자인과 로직을 분리시키기 위해 사용하여야 할 클래스 유형은 아래와 같다
 * 				M - 중립적인 모델이므로 순수 java 클래스로 작성 
 * 				V - 웹상의 디자인을 표현해야 하므로, html, jsp로 작성 
 * 				C - 클라이언트의 요청을 받아야 하고, 오직  javaEE 서버에서만 실행될 수 있어야 하므로, 서블릿임 
...
public class MovieController{
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		//이 클래스의 목적은 컨트롤러이므로, 더이상 디자인 영역을 침해해서는 안됨, 즉 여기서 out.print() 를 시도한다는 것은
		//MVC중 View영역을 침범하게 됨(월권행위)
		//out.print(movie);

		String msg=manager.getAdvice(movie);
		
		//이때 이 요청과 연관된 세션이 드디어 생성되면서, 자동으로 session ID가
		//발급됨..또한 응답 정보 생성시 클라이언에케 쿠키로 session ID함께 전송됨
		//쿠키 - 영구적(Persistence cookie=하드디스크), 세션쿠키(메모리) 
		//HttpSession session=request.getSession();
		
		//영화에 대한 판단결과는, 세션이 죽을때까지 함께 생존할 수있으므로, 
		//이 요청이 종료되어도, 그 값을 유지할 수 있음 
		//session.setAttribute("msg", msg);
		
		//세션말고도, 데이터를 전달하는 또 다른 방법이 있는 포워딩을 이용해보자 
		//현재 들어온 요청에 대해 응답을 하지 않은 상태로, 또 다른 서블릿에에 요청을 전달함 
		//이때 지정된 result.jsp의 서블릿의 service()메서드가 호출!!
		request.setAttribute("msg", msg);//세션과 생명유지 시간만 틀릴뿐 사용방법은 같다!!
		//현재 서블릿에서 응답을 처리하지 않았기 때문에 request 는 죽지않고 result.jsp 의 서블릿까지 생명이 유지
		//위의 판단결과를 여기서 출력하면 MVC위배됨..따라서 판단결과를 별도의 디자인 영역에서 보여줘야 한다!!
