...
public class DispatcherServlet extends HttpServlet{
	...
	//init() 메서드 안에 명시된 매개변수인  ServletConfig은 단어에서 이미 느껴지듯..이 서블릿과 관련된 환경 정보를 갖고 잇는 객체이다
	public void init(ServletConfig config) {
		try {
			...
			//현재 웹애플리케이션이 이클립스 내부 톰켓으로 실행될지, 아니면 실제 서버에서 실행될지 개발자가 알필요가 없이 현재 애플리케이션을
			...
			props.load(fis);//프로퍼스티가 사용할 스트림 로드
	...
	//클라이언트의 요청 방식이 다양하므로, 어떤 요청방식으로 들어오더라도, 아래의 메서드 하나로 몰아넣으면 ,코드는 
	//메서드마다 재작성할 필요가 없다 
	public void doRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//request.setCharacterEncoding("utf-8");//추후 필터 단계로 올릴예정 
		...
		System.out.println("클라인트가 요청 시 사용한 uri는 "+uri);
		/*
		 * 아래의 코드에서 클라이언트의 모든 요청 마다 1:1 대응하는 if문으로 요청을 처리하게 되면, 역시나 요청의 수가 방대해질경우
		 * 유지보수성이 떨어짐...해결책?
		...
		*/
		...
		try {
			...
			//아래의 메서드 호출의 경우, 분명 부모 형인Controller형의 변수로 메서드를 호출하고는 있으나,
			//자바의 문법 규칙상 자식이 부모의 메서드를 오버라이드 한경우(업그레이드 한 것으로 간주하여) 자식의 메서드를 호출한다
			//즉 자료형은 부모형이지만, 동작은 자식 자료형으로할 경우 현실의 생물의 다양성을 반영하였다고 하여 다형성(=polymorphism)이라함
			...
			//하위 컨틀로러가 포원딩하라고 부탁한 경우엔 포워딩 처리..
	...
	//서블릿의 생명주기 메서드중, 서블릿이 소멸할때 호출되는 메서드인 destory()재정의 
