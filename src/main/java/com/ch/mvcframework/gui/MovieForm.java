...
/*
 * 영화와 관련된 판단을 해주는 공통 로직을 javaSE의 GUI 프로그래밍에서도 사용할 수 있는 지 테스트해보자 
...
public class MovieForm extends JFrame {
	...
	public MovieForm() {
		...
		for(String movie : movies) { //jdk 1.5 부터 지원되는 improved for(컬렉션 프레임웍과 친함)
			box.addItem(movie);
		}
		
		
		//부착전에 html 처럼 레이아웃을 먼저 설정 
		setLayout(new FlowLayout());//FlowLayout이란 수평, 수직의 직선으로 컴포넌트를 배치함, 
